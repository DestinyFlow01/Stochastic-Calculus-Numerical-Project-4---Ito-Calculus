...
void Particle::Initialization() { x.push_back(1);}
...
void Particle::Brownian_Integral(int N, double dt) {
	...
	//Generating Ito integral of 1 - int exp(s/2) sin(Bs) dBs from 0 to t
	vector<double> Brownian = x;
	vector<double> integral = x;
	for(int i = 1; i<x.size(); i++) {
		integral[i] = integral[i-1] + exp((i-1)*dt/2)*sin(Brownian[i-1])*dx[i-1];
		x[i] = 1-integral[i];
	}
}
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Brownian Integral 3).csv')
data_mean = pd.read_csv('1D trajectory of particles (Brownian Integral 3) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Brownian Integral 3) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Brownian Integral 3) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Brownian Integral 3) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs) 1D Trajectories')
plt.grid()
...
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs) 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance,  np.cosh(time_variance) - 1, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs) 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title('Heatmap of Covariance Matrix for Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs)')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title('Heatmap of Correlation Matrix for Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs)')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs)
def analytical_covariance(s, t, T=1.0):
    ...
    covariance = np.cosh(min_st) - 1
    return covariance

# Analytical correlation matrix for Brownian Integral (1 - Int_0^t exp(s/2)sin(Bs) dBs)
...
int main() {	
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Brownian Integral 3).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Brownian Integral 3) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Brownian Integral 3) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Brownian Integral 3) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Brownian Integral 3) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Brownian Integral 3.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Brownian Integral 3" -Wl,--enable-auto-import
	"Brownian Integral 3.exe"
    */
}