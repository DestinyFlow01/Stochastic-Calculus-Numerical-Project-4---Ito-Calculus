...
void Particle::Brownian_Integral(int N, double dt) {
	...
	//Generating Ito integral of 4Bs^3 dBs from 0 to t
	vector<double> Brownian = x;
	for(int i = 1; i<x.size(); i++) {
		double Bs_cubed = pow(Brownian[i-1],3);
		x[i] = x[i-1] + 4*Bs_cubed*dx[i-1];
//		x[i] = x[i-1] + Brownian[i]*dx[i-1];
	}
}
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Brownian Integral 1).csv')
data_mean = pd.read_csv('1D trajectory of particles (Brownian Integral 1) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Brownian Integral 1) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Brownian Integral 1) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Brownian Integral 1) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Brownian Integral (Int_0^t 4Bs^3 dBs) 1D Trajectories')
plt.grid()
...
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Brownian Integral (Int_0^t 4Bs^3 dBs) 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance, 60*time_variance**4, 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Brownian Integral (Int_0^t 4Bs^3 dBs) 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title('Heatmap of Covariance Matrix for Brownian Integral (Int_0^t 4Bs^3 dBs)')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title('Heatmap of Correlation Matrix for Brownian Integral (Int_0^t 4Bs^3 dBs)')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Brownian Integral (Int_0^t 4Bs^3 dBs)
def analytical_covariance(s, t, T=1.0):
    ...
    covariance = 60*np.minimum.outer(s, t)**4
    return covariance

# Analytical correlation matrix for Brownian Integral (Int_0^t 4Bs^3 dBs)
...
int main() {	
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Brownian Integral 1).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Brownian Integral 1) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Brownian Integral 1) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Brownian Integral 1) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Brownian Integral 1) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Brownian Integral 1.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Brownian Integral 1" -Wl,--enable-auto-import
	"Brownian Integral 1.exe"
    */
}