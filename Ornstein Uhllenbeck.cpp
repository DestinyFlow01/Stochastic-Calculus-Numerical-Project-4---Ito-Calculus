...
void Particle::Brownian_Integral(int N, double dt) {
	...
	//Generating Ito integral of Ornstein Uhllenbeck from 0 to t
	vector<double> integral = x;
	for(int i = 1; i<x.size(); i++) {
		integral[i] = integral[i-1] + exp((i-1)*dt)*dx[i-1];
		x[i] = exp(-i*dt)*integral[i];
	}
}
...
# Read the CSV file
data_particle = pd.read_csv('1D trajectory of particles (Ornstein Uhllenbeck).csv')
data_mean = pd.read_csv('1D trajectory of particles (Ornstein Uhllenbeck) Mean.csv')
data_variance = pd.read_csv('1D trajectory of particles (Ornstein Uhllenbeck) Variance.csv')
data_covariance = pd.read_csv('1D trajectory of particles (Ornstein Uhllenbeck) Covariance.csv')
data_correlation = pd.read_csv('1D trajectory of particles (Ornstein Uhllenbeck) Correlation.csv')
...
plt.xlabel('Time')
plt.ylabel('Position')
plt.title('Ornstein Uhllenbeck 1D Trajectories')
plt.grid()
...
plt.xlabel('Time')
plt.ylabel('Mean')
plt.title('Ornstein Uhllenbeck 1D Trajectories (Mean)')
plt.grid()
plt.show(block=False)  

plt.figure(3)
plt.plot(time_variance, data_variance['variance'])
plt.plot(time_variance,  0.5*(1 - np.exp(-2*time_variance)), 'k--', linewidth=2,)
plt.xlabel('Time')
plt.ylabel('Variance')
plt.title('Ornstein Uhllenbeck 1D Trajectories (Variance)')
plt.grid()
plt.show(block=False)  
...
plt.title('Heatmap of Covariance Matrix for Ornstein Uhllenbeck')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
plt.colorbar(label='Value')
plt.title('Heatmap of Correlation Matrix for Ornstein Uhllenbeck')
plt.xlabel("Time")
plt.ylabel("Time")
plt.tight_layout()
plt.show(block=False)
...
# Define the analytical covariance for Ornstein Uhllenbeck
def analytical_covariance(s, t, T=1.0):
    ...
    covariance =  0.5*np.exp(-(max_st - min_st))*(1-np.exp(-2*min_st))
    return covariance

# Analytical correlation matrix for Ornstein Uhllenbeck
...
int main() {	
	...
	cout<<"- Trajectories : ";
	string name = "1D trajectory of particles (Ornstein Uhllenbeck).csv"; OutputCSV_1D(name, dt, particles, N);   
	cout<<"Done\n"; 	
	
	cout<<"- Mean : ";
	name = "1D trajectory of particles (Ornstein Uhllenbeck) Mean.csv"; OutputCSV_StatQuan1D(name, dt, mean, N, "mean");
	cout<<"Done\n";
	
	cout<<"- Variance : ";
	name = "1D trajectory of particles (Ornstein Uhllenbeck) Variance.csv"; OutputCSV_StatQuan1D(name, dt, variance, N, "variance");
	cout<<"Done\n";
	
	cout<<"- Covariance : ";
	name = "1D trajectory of particles (Ornstein Uhllenbeck) Covariance.csv"; OutputCSV_StatQuan2D(name, dt, covariance, N, "Cov");
	cout<<"Done\n";
	
	cout<<"- Correlation : ";
	name = "1D trajectory of particles (Ornstein Uhllenbeck) Correlation.csv"; OutputCSV_StatQuan2D(name, dt, correlation, N, "Corr");
    ...
    /*
    Type this in CMD
    g++ "Ornstein Uhllenbeck.cpp" -IC:\Python313\include -LC:\Python313\libs -lpython313 -o "Ornstein Uhllenbeck" -Wl,--enable-auto-import
	"Ornstein Uhllenbeck.exe"
    */
}